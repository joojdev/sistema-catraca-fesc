...
turnstile.on('close', (hadError) => {
    logger.warn(`Connection closed ${hadError ? 'with' : 'without'} error(s). Attempting reconnect…`);
    turnstile.connect(); // tenta reconectar indefinidamente
});

// ------------------------- Handlers de Eventos --------------------------
async function handleRFID(_eventCode: number, data: string[], message: Message, currentDate: Date) {
    ...
    // ---------------------- Consulta Tag -------------------------------
    const tag: Tag | null = await prisma.tag.findUnique({ where: { credential: tagId } });
...
async function handleTurn(_eventCode: number, message: Message, currentDate: Date) {
    if (!waitingToTurn) return; // nada pendente

    const tag: Tag | null = await prisma.tag.findUnique({ where: { credential: waitingToTurn.tagId } });
...
// ------------------------- Bootstrap -------------------------------------
main()
    .catch((err) => logger.error({ err }, 'Fatal error in main loop'));
